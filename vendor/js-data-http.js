...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function xhrAdapter(resolve, reject, config) {
	  ...
	  var upload_events = config.__uploadEvents||{};
	  var download_events = config.__downloadEvents||{};
	  //Axios upload and download event backport
	  if (config.onUploadProgress)
	  	upload_events.progress = config.onUploadProgress;
	  if (config.onDownloadProgress)
	  	download_events.progress = config.onDownloadProgress;
	  //Apply event handlers
	  for (var event in download_events)
	    request.addEventListener(event, download_events[event]);
	  for (var event in upload_events)
	    request.upload.addEventListener(event, upload_events[event]);
